            ...
            Form {
                ...
                Section {
                    ...
                    if let historyMessage {
                        Text(verbatim: historyMessage).font(.footnote).foregroundStyle(.secondary)
                    }
                } header: {
                    Text("抓取过往公演", bundle: .kit)
                } footer: {
                ...
                Section {
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        Text("登录后可自动整理多日公演重点、识别售票与通贩链接。摘要为 AI 生成，非官方资料。", bundle: .kit)
                        Text("公演资料、关注、手动申请状态、提醒和卡片设置都保存在本机。检查官方资料不需要账号或服务器安装身份。若登录 ChatGPT 助手，会将官网页面文字发送到 OpenAI 生成摘要；凭据保存在本机钥匙串。", bundle: .kit)
                    }
                }
            }
